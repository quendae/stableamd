...
if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) {
    throw 'Model folder browsing is available on Windows only.'
}
...
try {
    $folder = $shell.BrowseForFolder(0, 'Select a folder containing Stable Diffusion checkpoints', 0, 0)
    ...
    if (-not (Test-Path $selected -PathType Container)) {
        throw "Selected model folder does not exist: '$selected'."
    }
